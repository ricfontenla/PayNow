...
describe PixAccount do
  it {
    should validate_presence_of(:pix_key)
      .with_message('não pode ficar em branco')
  }
  it {
    should validate_presence_of(:bank_code)
      .with_message('não pode ficar em branco')
  }

  it {
    should validate_length_of(:pix_key).is_equal_to(20)
                                       .with_message('não possui o tamanho esperado (20 caracteres)')
  }

  it { should allow_value('1234567890qwertyuiop').for(:pix_key) }
  it {
    should_not allow_value('1234567890qwertyuio#').for(:pix_key)
  ...
  context 'registers boleto_account' do
    it 'fields should be unique' do
      ...
      PixAccount.create!(pix_key: '12345abcde67890FGHIJ',
                         ...
                         company: company,
                         payment_method: pix)
      User.create!(email: 'jane_doe@codeplay.com.br',
                   password: '123456',
                   role: 0,
                   company: company)
